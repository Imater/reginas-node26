...
//    $scope.group1 = "manager_id";
    ...
    var jsLoadBig = function($scope) {
      var dfd = $q.defer();
        $http({url:'/api/v1/stat_big3',method: "GET", params: { brand: $scope.fpk.brand, today: $scope.fpk.today_date, d1:$scope.fpk.d1, d2:$scope.fpk.d2, group1: $scope.group1, group2: $scope.group2, manager_filter: $scope.fpk.manager_filter }}).then(function(result){
    ...
    $scope.jsColDates = function() {
          ...
          while( d<=d2 ) {
            ...
            if($scope.show_days) col_dates.push({date: d});
            d = new Date(d);
          ...
          col_dates = _.map( col_dates, function(col_date){
            ...
            if(col_date.date) {
                ...
                answer.date = { date: dt.replace(/-/ig, "."), title: title, int_date: col_date.date };
                ...
                answer.date = { month: true, date: dt.replace(/-/ig, "."), title: title, int_date: col_date.month };
            }
            return answer;
          });
...
*/
    ...
    $scope.jsFindStat = _.memoize( function(group_line, col_date) {
        ...
        return answer;
    }, function(group_line, col_date) { return group_line.time+JSON.stringify(col_date) });
    ...
    }, function(top){ return JSON.stringify(top) });
