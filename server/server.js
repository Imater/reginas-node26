...
exports.getDo = function(request,response) {
  ...
  jsCheckToken(request.query.token, response).done(function(user_id){
    ...
    if( cal_type == 'all_do' ) {
      ...
      var query = 'SELECT 1_do.*, 1_clients.id, 1_clients.fio, 1_models.short, 1_users.fio man FROM 1_do LEFT JOIN 1_clients ON 1_do.client = 1_clients.id LEFT JOIN 1_models ON 1_models.id =1_clients.model  LEFT JOIN 1_users ON 1_do.manager_id = 1_users.id WHERE 1_do.date2<= DATE_ADD(NOW(), INTERVAL 15 DAY) AND 1_do.checked != "0000-00-00 00:00:00" AND 1_do.host_id = '+user_id+' AND host_id <> 1_do.manager_id AND (true OR brand = ?) ORDER by checked DESC LIMIT 150';            
    }
    console.info("is_error_here? = ",query);
    if(brand)
...
var jsCheckDublicate = function(client) {
  ...
  var phones = "'" + (client.phone1?client.phone1:"!!!") + "', '" +
                 (client.phone2?client.phone2:"!!!") + "', '" +
                 (client.phone3?client.phone3:"!!!") + "', '" +
                 (client.phone4?client.phone4:"!!!") + "'";
  var vin = client.vin;
  ...
  pool.query(query, function (err, clients, fields) {
    ...
    $.each(clients, function(j, cl){
      if( (client.id != cl.id) && (client.manager_id != cl.manager_id) ) {
          dublicates.push( {brand: cl.brand, 
            fio: cl.fio, 
            model: cl.model, 
